...
void cg::renderer::dx12_renderer::init()
{
	...
	world_view_projection =
		camera->get_dxm_projection_matrix() * camera->get_dxm_view_matrix();
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	rs_descriptor.Init_1_1(_countof(root_parameters), 
		root_parameters, 0, nullptr, rs_flags);
	...
	pso_descriptor.RasterizerState = 
	...
	THROW_IF_FAILED(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
		command_allocators[0].Get(), pipeline_state.Get(), 
	...
	THROW_IF_FAILED(vertex_buffer->Map(0, &read_range, 
		reinterpret_cast<void**>(&vertex_data_begin)));
	
	memcpy(vertex_data_begin, vertex_buffer_data->get_data(),
		vertex_buffer_size);
	...
	memcpy(constant_buffer_data_begin, 
		&world_view_projection, sizeof(world_view_projection));
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	command_list->DrawInstanced(model->get_vertex_buffer()->get_number_of_elements(),
		1, 0, 0);
